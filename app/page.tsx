  ...
  useEffect(() => {
    ...
    async function loadPublishedGuide() {
      try {
        ...
        if (!response.ok) {
          throw new Error("Guide not published");
        }
        ...
        if (isMounted) {
          setContent(payload.translations);
          setGuideLoadState("published");
        }
      } catch {
        if (isMounted) {
          setContent(fallbackContent);
          setGuideLoadState("fallback");
        }
      }
    }
  ...
  return (
    <main className="min-h-screen bg-emerald-50 text-slate-950">
      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-6 px-5 py-6 sm:px-8 lg:py-10">
        {isGuideLoading ? (
          <section className="flex min-h-[360px] items-center justify-center rounded-3xl bg-white p-6 text-center shadow-md shadow-emerald-900/10 ring-1 ring-emerald-100 sm:p-8">
            <div>
              <div className="mx-auto mb-5 h-10 w-10 animate-spin rounded-full border-4 border-emerald-100 border-t-emerald-700" />
